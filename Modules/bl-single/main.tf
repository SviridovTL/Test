...
# Subnet in AZ 1
resource "aws_subnet" "bl_sn_1" {

  vpc_id                      = "${var.vpc_id}"
  cidr_block                  = "${var.bl_sn_pb_1}"
  availability_zone           = "${var.region}${var.bl_az_1}"

  tags {
     Name                     = "${var.env}-${var.system}-bl-pb-sn-${var.bl_az_1}"
  }
...
################################################################################
resource "aws_route_table_association" "bl_rt_ast_1" {

  subnet_id                  = "${aws_subnet.bl_sn_1.id}"
...
resource "aws_instance" "bl_az_1" {
  ...
  availability_zone       = "${var.region}${var.bl_az_1}"
  subnet_id               = "${aws_subnet.bl_sn_1.id}"
  private_ip              = "${var.bl_az1_ip}"
  vpc_security_group_ids  = ["${aws_security_group.bl_sg.id}"]
  associate_public_ip_address               = true
  # system disk
  root_block_device {
        #device_name       = "/dev/sda1"
        volume_type       = "gp2"
        volume_size       = "${var.bl_ebs_sys}"
        delete_on_termination = true
  }

  tags {
    Name                  = "${var.env}-${var.system}-bl-1${var.bl_az_1}"
  }
}
